...
// or null otherwise.
    ...
    public T remove() {
        ...
        int randomNo = (int )(Math.random() * numberOfEntries + 1   );
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        //reference to first node of aBag(to which we are comparing)
        
        Node a=aBag.firstNode;
        
        // reference to first node of this bag
        Node b=firstNode;
        // counter helps in checking equality
        int count=0;
        // if both bags are of different size 
        if(aBag.numberOfEntries!=numberOfEntries)
        	return false;
        
        // check each element of aBag to each element of this bag
        while(a!=null)
        	{ for( b=firstNode;b!=null;b=b.next)
        	{
        		if(a.data==b.data)
        			break;
        	}
        	// if an element is found in this bag
        	if(b==null)
        		return false;
        	a=a.next;
        	count++;
        	}
        // if all elements are being matched
        if(count==numberOfEntries)
        	return true;
        // if all elements are not being matched
        return result;
    }  // end equals
...
//getting reference to first node
        Node p=firstNode;
        // adding duplicate for each node
        while(p!=null)
        {
        	//adding a duplicate of current refered node
        	add(p.data);
        	p=p.next;
        }
        return success;
    ...
    public void removeDuplicates() {
        ...
        // create a new bag
    	LinkedBag<T> unique=new LinkedBag<T>();
        //get reference to first node
    	Node p=firstNode;
        
        //traverse till end and start inserting at beginning
    	for(p=firstNode;p!=null;p=p.next)
    	{
    		if(!unique.contains(p.data))
    			{
    			unique.add(p.data);
    			}
    	}
   
        // point current bag to new bag
    	firstNode=unique.firstNode;
    	numberOfEntries=unique.numberOfEntries;
