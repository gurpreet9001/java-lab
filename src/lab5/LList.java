    ...
    */
     ...
     */
     public void reverse() {
         ...
         //start "i" from the start and "j" from the end towards the middle
         for(i=1; i <= j ; i++,j--)
         	{
                // store the current entry into the temp node
         	T temp = this.getEntry(i);
                //copy the current rightmost node to leftmost node
         	this.replace(i, this.getEntry(j));
                //copy temp node to current rightmost node
         	this.replace(j, temp);
     		}     
    }
        ...
        for(i=1; i < numberOfEntries ; i++)
        	{
                // store the current entry into the temp node
        	T temp = this.getEntry(i);
                //copy the next node to current node
        	this.replace(i, this.getEntry(i+1));
                //copy the temp node to next node
